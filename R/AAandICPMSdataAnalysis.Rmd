...
AAsample_data <- merge(AA_tidy_filter, Cr_data) %>% 
  unique()%>%
  mutate(AAconc_blanked = AA_conc*(total_volume/1000)/(mass_of_soil/1000)*101,
         AAconc_blanked_error = AAconc_blanked*
           sqrt((AA_conc_error/AA_conc)^2 + 
                  (dil_e/101)^2 +
                  (mass_e/mass_of_soil)^2 +
                  (vol_e/total_volume)^2),
         conc_unblanked = xCr*(total_volume/1000)/(mass_of_soil/1000)*101,
         conc_unblanked_error = conc_unblanked*
...
```{r}
AA_sample_data["sample_key"] <- Cr_data$sample_key
AA_sample_data["site"] <- Cr_data$site

AA_d1 <- subset(AA_sample_data, select = -c(conc_unblanked, conc_unblanked_error))
AA_data_conc <- AA_d1 %>% 
  rename(
    AA_conc = AAconc_blanked,
    AA_conc_error = AAconc_blanked_error
    )
AA_data_conc
```
